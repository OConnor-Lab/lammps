/* ----------------------------------------------------------------------
   LAMMPS - Large-scale Atomic/Molecular Massively Parallel Simulator
   https://www.lammps.org/, Sandia National Laboratories
   LAMMPS development team: developers@lammps.org

   Copyright (2003) Sandia Corporation.  Under the terms of Contract
   DE-AC04-94AL85000 with Sandia Corporation, the U.S. Government retains
   certain rights in this software.  This software is distributed under
   the GNU General Public License.

   See the README file in the top-level LAMMPS directory.
------------------------------------------------------------------------- */

/* ----------------------------------------------------------------------
   Contributing author: Loukas D. Peristeras (Scienomics SARL)
   [ based on angle_cosine_squared.cpp Naveen Michaud-Agrawal (Johns Hopkins U)]
------------------------------------------------------------------------- */

#include "angle_snsq.h"

#include "atom.h"
#include "comm.h"
#include "domain.h"
#include "error.h"
#include "force.h"
#include "math_const.h"
#include "memory.h"
#include "neighbor.h"

#include <cmath>
#include <cstring>

using namespace LAMMPS_NS;
using namespace MathConst;

static constexpr double SMALL = 0.0001;

/* ---------------------------------------------------------------------- */

AngleSNSQ::AngleSNSQ(LAMMPS *lmp) : Angle(lmp)
{
  born_matrix_enable = 1;
  A = nullptr;
  N = nullptr;
  cut = nullptr;
}

/* ---------------------------------------------------------------------- */

AngleSNSQ::~AngleSNSQ()
{
  if (allocated) {
    memory->destroy(setflag);
    memory->destroy(A);
    memory->destroy(N);
    memory->destroy(cut);
  }
}

/* ---------------------------------------------------------------------- */

void AngleSNSQ::compute(int eflag, int vflag)
{
  int i1, i2, i3, n, type;
  double delx1, dely1, delz1, delx2, dely2, delz2;
  double eangle, f1[3], f3[3];
  double term, sgn;
  double rsq1, rsq2, r1, r2, c, cn, th, nth, a, a11, a12, a22;
  double npi, s, gamma;

  eangle = 0.0;
  ev_init(eflag, vflag);

  double **x = atom->x;
  double **f = atom->f;
  int **anglelist = neighbor->anglelist;
  int nanglelist = neighbor->nanglelist;
  int nlocal = atom->nlocal;
  int newton_bond = force->newton_bond;

  for (n = 0; n < nanglelist; n++) {
    i1 = anglelist[n][0];
    i2 = anglelist[n][1];
    i3 = anglelist[n][2];
    type = anglelist[n][3];

    // 1st bond

    delx1 = x[i1][0] - x[i2][0];
    dely1 = x[i1][1] - x[i2][1];
    delz1 = x[i1][2] - x[i2][2];

    rsq1 = delx1 * delx1 + dely1 * dely1 + delz1 * delz1;
    r1 = sqrt(rsq1);

    // 2nd bond

    delx2 = x[i3][0] - x[i2][0];
    dely2 = x[i3][1] - x[i2][1];
    delz2 = x[i3][2] - x[i2][2];

    rsq2 = delx2 * delx2 + dely2 * dely2 + delz2 * delz2;
    r2 = sqrt(rsq2);

    // angle (cos and sin)

    c = delx1 * delx2 + dely1 * dely2 + delz1 * delz2;
    c /= r1 * r2;

    if (c > 1.0) c = 1.0;
    if (c < -1.0) c = -1.0;

    // force & energy

    gamma = acos(c);

    //Check for cutoff
    if (gamma <= cut[type]){
      term = 0.0;
      if (eflag) eangle = term;
      f1[0] = 0.0;
      f1[1] = 0.0;
      f1[2] = 0.0;
      f3[0] = 0.0;
      f3[1] = 0.0;
      f3[2] = 0.0;
    }else{
      //Define theta as angle relative to straight config 
      //The standard angle is gamma and theta = pi -gamma
      th  = MY_PI - gamma;
      nth = N[type] * th;
      npi = N[type] * MY_PI;

      //Tabulate energy:
      term = -A[type]*sin(nth)*sin(nth);
      if (eflag) eangle = term;

      //linearize if theta is too small
      c = cos(th);
      s = sqrt(1-c*c);
      if (s<SMALL){
        a = 2*A[type]*N[type]*N[type];
      }else{
        a = A[type]*N[type]*sin(2*nth)/s;
      }
      c = cos(gamma);
      a11 = -a * c / rsq1; //
      a12 =  a / (r1 * r2);
      a22 = -a * c / rsq2;
      f1[0] = a11 * delx1 + a12 * delx2;
      f1[1] = a11 * dely1 + a12 * dely2;
      f1[2] = a11 * delz1 + a12 * delz2;
      f3[0] = a22 * delx2 + a12 * delx1;
      f3[1] = a22 * dely2 + a12 * dely1;
      f3[2] = a22 * delz2 + a12 * delz1;
    }
    // apply force to each of 3 atoms

    if (newton_bond || i1 < nlocal) {
      f[i1][0] += f1[0];
      f[i1][1] += f1[1];
      f[i1][2] += f1[2];
    }

    if (newton_bond || i2 < nlocal) {
      f[i2][0] -= f1[0] + f3[0];
      f[i2][1] -= f1[1] + f3[1];
      f[i2][2] -= f1[2] + f3[2];
    }

    if (newton_bond || i3 < nlocal) {
      f[i3][0] += f3[0];
      f[i3][1] += f3[1];
      f[i3][2] += f3[2];
    }

    if (evflag)
      ev_tally(i1, i2, i3, nlocal, newton_bond, eangle, f1, f3, delx1, dely1, delz1, delx2, dely2,
               delz2);
  }
}

/* ---------------------------------------------------------------------- */

void AngleSNSQ::allocate()
{
  allocated = 1;
  int n = atom->nangletypes;
  memory->create(A, n + 1, "angle:A");
  memory->create(N, n + 1, "angle:N");
  memory->create(cut, n + 1, "angle:cut");
  memory->create(setflag, n + 1, "angle:setflag");
  for (int i = 1; i <= n; i++) setflag[i] = 0;
}

/* ----------------------------------------------------------------------
   set coeffs for one or more types
------------------------------------------------------------------------- */

void AngleSNSQ::coeff(int narg, char **arg)
{
  if (narg != 3) error->all(FLERR, "Incorrect args for angle coefficients" + utils::errorurl(21));
  if (!allocated) allocate();

  int ilo, ihi;
  utils::bounds(FLERR, arg[0], 1, atom->nangletypes, ilo, ihi, error);
  double A_one = utils::numeric(FLERR, arg[1], false, lmp);
  double N_one = utils::numeric(FLERR, arg[2], false, lmp);
  //double cut_one = utils::numeric(FLERR, arg[3], false, lmp);
  if (N_one < 1.0) error->all(FLERR,"Incorrect args for angle coefficients" + utils::errorurl(21));

  double cut_one = MY_PI - MY_PI/N_one;

  int count = 0;
  for (int i = ilo; i <= ihi; i++) {
    A[i] = A_one;
    N[i] = N_one;
    cut[i] = cut_one;
    setflag[i] = 1;
    count++;
  }

  if (count == 0) error->all(FLERR, "Incorrect args for angle coefficients" + utils::errorurl(21));
}

/* ---------------------------------------------------------------------- */

double AngleSNSQ::equilibrium_angle(int i)
{
  return (MY_PI - MY_PI / N[i]);
}

/* ----------------------------------------------------------------------
   proc 0 writes out coeffs to restart file
------------------------------------------------------------------------- */

void AngleSNSQ::write_restart(FILE *fp)
{
  fwrite(&A[1], sizeof(double), atom->nangletypes, fp);
  fwrite(&N[1], sizeof(double), atom->nangletypes, fp);
  fwrite(&cut[1], sizeof(double), atom->nangletypes, fp);
}

/* ----------------------------------------------------------------------
   proc 0 reads coeffs from restart file, bcasts them
------------------------------------------------------------------------- */

void AngleSNSQ::read_restart(FILE *fp)
{
  allocate();

  if (comm->me == 0) {
    utils::sfread(FLERR, &A[1], sizeof(double), atom->nangletypes, fp, nullptr, error);
    utils::sfread(FLERR, &N[1], sizeof(double), atom->nangletypes, fp, nullptr, error);
    utils::sfread(FLERR, &cut[1], sizeof(double), atom->nangletypes, fp, nullptr, error);
  }
  MPI_Bcast(&A[1], atom->nangletypes, MPI_DOUBLE, 0, world);
  MPI_Bcast(&N[1], atom->nangletypes, MPI_DOUBLE, 0, world);
  MPI_Bcast(&cut[1], atom->nangletypes, MPI_DOUBLE, 0, world);

  for (int i = 1; i <= atom->nangletypes; i++) setflag[i] = 1;
}

/* ----------------------------------------------------------------------
   proc 0 writes to data file
------------------------------------------------------------------------- */

void AngleSNSQ::write_data(FILE *fp)
{
  for (int i = 1; i <= atom->nangletypes; i++) fprintf(fp, "%d %g %g\n", i, A[i], N[i]);
}

/* ---------------------------------------------------------------------- */

double AngleSNSQ::single(int type, int i1, int i2, int i3)
{
  double **x = atom->x;

  double delx1 = x[i1][0] - x[i2][0];
  double dely1 = x[i1][1] - x[i2][1];
  double delz1 = x[i1][2] - x[i2][2];
  domain->minimum_image(FLERR, delx1, dely1, delz1);
  double r1 = sqrt(delx1 * delx1 + dely1 * dely1 + delz1 * delz1);

  double delx2 = x[i3][0] - x[i2][0];
  double dely2 = x[i3][1] - x[i2][1];
  double delz2 = x[i3][2] - x[i2][2];
  domain->minimum_image(FLERR, delx2, dely2, delz2);
  double r2 = sqrt(delx2 * delx2 + dely2 * dely2 + delz2 * delz2);

  double c = delx1 * delx2 + dely1 * dely2 + delz1 * delz2;
  c /= r1 * r2;
  if (c > 1.0) c = 1.0;
  if (c < -1.0) c = -1.0;
  double gamma = acos(c);
  if (gamma<=cut[type]){
    double eng = 0.0;
    return eng;
  }else{
    double nth = N[type] * (MY_PI-gamma);
    double npi = N[type] * MY_PI;
    double eng = -A[type]*sin(nth)*sin(nth);
    return eng;
  }
}

/* ---------------------------------------------------------------------- */

//void AngleSNSQ::born_matrix(int type, int i1, int i2, int i3, double &du, double &du2)
//{
//  double **x = atom->x;
//
//  double delx1 = x[i1][0] - x[i2][0];
//  double dely1 = x[i1][1] - x[i2][1];
//  double delz1 = x[i1][2] - x[i2][2];
//  domain->minimum_image(FLERR, delx1,dely1,delz1);
//  double r1 = sqrt(delx1*delx1 + dely1*dely1 + delz1*delz1);
//
//  double delx2 = x[i3][0] - x[i2][0];
//  double dely2 = x[i3][1] - x[i2][1];
//  double delz2 = x[i3][2] - x[i2][2];
//  domain->minimum_image(FLERR, delx2,dely2,delz2);
//  double r2 = sqrt(delx2*delx2 + dely2*dely2 + delz2*delz2);
//
//  double c = delx1*delx2 + dely1*dely2 + delz1*delz2;
//  c /= r1*r2;
//  if (c > 1.0) c = 1.0;
//  if (c < -1.0) c = -1.0;
//  double theta = acos(c);
//  double npi = N[type]*MY_PI;
//  double nth = N[type]*theta;
//
//
//  if (theta<cut[type]){
//    du = 0.0;
//    du2 = 0.0;
//  }else{
//    du = A[type]*N[type]*sin(2*(npi-nth))/sin(theta);
//    du2 = k[type] * C[type] * N[type] * (cos(theta) * sin(N[type] * theta)
//                    - N[type] * sin(theta) * cos(N[type] * theta)) / pow(sin(theta),3);
//  }
//}

/* ----------------------------------------------------------------------
   return ptr to internal members upon request
------------------------------------------------------------------------ */

void *AngleSNSQ::extract(const char *str, int &dim)
{
  dim = 1;
  if (strcmp(str, "A") == 0) return (void *) A;
  if (strcmp(str, "N") == 0) return (void *) N;
  if (strcmp(str, "cut") == 0) return (void *) cut;
  return nullptr;
}
